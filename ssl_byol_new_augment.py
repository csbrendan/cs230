...
############### DATASETS - load the data via method from chestmnist.py
...
val_loader = DataLoader(dataset=TEST_DATASET, batch_size=2*BATCH_SIZE, shuffle=False) 
...
class CustomTransform(nn.Module):
    def __init__(self, cutout, adjust_contrast, adjust_brightness):
        super().__init__()
        self.cutout = cutout
...
def cutout(image, cutout_size):
    ...
    upper = random.randint(0, height - cutout_size)
    right = left + cutout_size
    lower = upper + cutout_size
    image[:, upper:lower, left:right] = 0
    return image

def random_adjust_contrast(image, strength_range=(0.5, 1.5)):
    contrast_factor = random.uniform(strength_range[0], strength_range[1])
    image = TF.adjust_contrast(image, contrast_factor)
    return image


def random_adjust_brightness(image, strength_range=(-0.2, 0.2)):
    brightness_factor = random.uniform(strength_range[0], strength_range[1])
    if brightness_factor >= 0:
        brightness_factor = 1 + brightness_factor
    else:
        brightness_factor = 1 / (1 - brightness_factor)
    image = TF.adjust_brightness(image, brightness_factor)
    return image


def default_augmentation(image_size: Tuple[int, int] = (28, 28)) -> nn.Module:
    cut_size = random.randint(1, 8)
    return nn.Sequential(
        transforms.Resize(size=image_size),
        transforms.RandomApply([transforms.ColorJitter(0.8, 0.8, 0.8, 0.2)], p=0.8),
        transforms.RandomGrayscale(p=0.2),
        transforms.RandomHorizontalFlip(p=0.5),
        transforms.RandomResizedCrop(size=image_size, scale=(0.08, 1.0), ratio=(0.75, 1.3333333333333333)),
        CustomTransform(
            lambda img: cutout(img, cutout_size=cut_size),
...
trainer = pl.Trainer(
    max_epochs=1000,
    accumulate_grad_batches=2048 //256,  # // 128
)
...
torch.save(model.state_dict(), 'byol2_pretrained_chestmnist_new_augment_batchsize_256_1000_epochs.pth')
